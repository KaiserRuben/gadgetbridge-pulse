import { ANOMALIES_FACTORS } from "../../confidence-weights.ts";
import {
  COMPARISON_TYPED,
...
/**
 * snapshot/anomalies — strict JSON Schema (v2, dual-consumer).
 *
 ...
 * fields below.
 *
 * NOTE: anomalies follows the SAME v2 schema as the other domains for
 * consistency. Drivers are framed as anomaly counts (active, watching,
 * data notes) with directions reflecting whether the count is concerning.
 */
export const AnomaliesSnapshotSchema = {
  type: "object",
  ...
  properties: {
    ...
    // ── confidence ─────────────────────────────────────────────────────
    confidence: confidenceBlockSchema(ANOMALIES_FACTORS),
